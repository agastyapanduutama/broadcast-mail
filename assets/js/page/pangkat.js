$(document).ready(function () {
	table = $('#list_pangkat').DataTable({
		"processing": true,
		"serverSide": true,
		"order": [],

		"ajax": {
			"url": baseUrl + 'admin/pangkat/data',
			"type": "POST",
			"complete": function () {
				cekPilihanNa()
			},
...
$('#list_pangkat').on('click', '#edit', function () {
	let id = $(this).data('id');
	$.ajax({
		url: baseUrl + 'admin/pangkat/get/' + id,
		type: "GET",
		success: function (result) {
			response = JSON.parse(result)
			$("#idData").val(response.id)
			$("#pangkat1").val(response.nama_pangkat)
...
$('#list_pangkat').on('click', '#delete', function () {
	let id = $(this).data('id');
	confirmSweet("Data akan terhapus secara permanen !")
		.then(result => {
			if (result) {
				$.ajax({
					url: baseUrl + 'admin/pangkat/delete/' + id,
					type: "GET",
					success: function (result) {
						response = JSON.parse(result)
...
$("#formAddpangkat").submit(function (e) {
	e.preventDefault();
	$.ajax({
		url: baseUrl + "admin/pangkat/insert",
		type: "post",
...
$("#formEditpangkat").submit(function (e) {
	e.preventDefault();
	$.ajax({
		url: baseUrl + "admin/pangkat/update",
		type: "post",
		data: new FormData(this),
		processData: false,
		contentType: false,
...
$('#list_pangkat').on('click', '#on', function () {
    let id = $(this).data('id');
    confirmSweet("Data akan dinon-aktifkan  !")
        .then(result => {
            if (result) {
                $.ajax({
                    url: baseUrl + 'admin/pangkat/set/' + id + "/off",
                    type: "GET",
                    success: function (result) {
                        response = JSON.parse(result)
                        if (response.status == 'ok') {
...
$('#list_pangkat').on('click', '#off', function () {
    let id = $(this).data('id');
    confirmSweet("Data akan diaktifkan  !")
        .then(result => {
            if (result) {
                $.ajax({
                    url: baseUrl + 'admin/pangkat/set/' + id + "/on",
                    type: "GET",
                    success: function (result) {
                        response = JSON.parse(result)
                        if (response.status == 'ok') {
...
$("#btnAllOff").on("click", function () {
	...
	if (dipilih == '') {
		...
		confirmSweet(`${jml.length - 1} Akun akan dinon-aktifkan  !`).then(respon => {
			if (respon) {
				$.ajax({
					url: `${baseUrl}/admin/pangkat/allOff`,
					data: {
						id: dipilih
					},
					type: "post",
...
$("#btnAllOn").on("click", function () {
	...
	if (dipilih == '') {
		...
		confirmSweet(`${jml.length - 1} Akun akan diaktifkan  !`).then(respon => {
			if (respon) {
				$.ajax({
					url: `${baseUrl}/admin/pangkat/allOn`,
					data: {
						id: dipilih
					},
					type: "post",
...
$("#btnAllHapus").on("click", function () {
	...
	if (dipilih == '') {
		...
		confirmSweet(`${jml.length - 1} Akun akan dihapus  !`).then(respon => {
			if (respon) {
				$.ajax({
					url: `${baseUrl}/admin/pangkat/allDelete`,
					data: {
						id: dipilih
					},
					type: "post",
